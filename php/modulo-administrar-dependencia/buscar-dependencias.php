...
// Consulta SQL para obtener las dependencias de esta página con opción de búsqueda
if (!empty($busqueda)) {
    // Si hay una cadena de búsqueda, ajustar la consulta SQL
    $sql = "SELECT id, nombre, estado FROM dependencia WHERE nombre LIKE ? LIMIT ?, ?";
    ...
    $sql = "SELECT id, nombre, estado FROM dependencia LIMIT ?, ?";
    $stmt = $conn->prepare($sql);
...
// Array para almacenar las dependencias encontradas
$dependencias = [];
...
$sql_total = "SELECT COUNT(*) AS total FROM dependencia";
if (!empty($busqueda)) {
    // Si hay una cadena de búsqueda, ajustar la consulta SQL
    $sql_total .= " WHERE nombre LIKE ?";
}
